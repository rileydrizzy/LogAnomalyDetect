...
def dataset_func(file_path, batch_size, shuffle_size, shuffle = True):
...
def test_func(i, j):
    nums = tf.range(i,j)
    nums = tf.data.Dataset.from_tensor_slices(nums)
    return nums
...
def main():
    ...
    temp = ''
    #main
    dataframes_set = load_spit(file_path=temp, target= temp)
    #zip dataframes and thier export path
    for dataset in dataframes_set:
        dataset = dataset.with_columns(pl.col('Target').apply(label_encoder, return_dtype=pl.Int32))
        dataset = dataset.with_columns(pl.col('Log').apply(clean_text_preprocess))
        save_to_parquet(dataset,temp)
